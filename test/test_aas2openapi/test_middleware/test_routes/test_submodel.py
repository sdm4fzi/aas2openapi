...
def test_create_submodel(client, example_aas_instance):
    ...
    response = client.post(url=f"/{class_name}/{example_aas_instance.id_}/{sm_class_name}", content=data)
    assert response.status_code == 200
    response = client.post(url=f"/{class_name}/{example_aas_instance.id_}/{sm_class_name}", content=data)
...
def test_get_submodel(client, example_aas_instance, example_submodel_instance):
    class_name = example_aas_instance.__class__.__name__
    response = client.get(url=f"/{class_name}/{example_aas_instance.id_}/BillOfMaterial")
    assert response.status_code == 200
...
def test_delete_submodel(client, example_aas_instance):
    class_name = example_aas_instance.__class__.__name__
    response = client.delete(url=f"/{class_name}/{example_aas_instance.id_}/ProcessModel")
    assert response.status_code == 200
    response = client.get(url=f"/{class_name}/{example_aas_instance.id_}/ProcessModel")
